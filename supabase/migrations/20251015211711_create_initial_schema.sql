/*
  ...
  ### 4. `ferramentas`
  Stores tools and equipment with complete tracking information
  ...
  - `owner_id` (uuid) - Reference to Host user
  - `descricao` (text) - Tool description
  - `nf` (text) - Invoice number
  - `nf_image_url` (text) - Invoice image URL
  - `data` (text) - Purchase/registration date
  - `valor` (numeric) - Tool value
  - `tempo_garantia_dias` (integer) - Warranty duration in days
  - `garantia` (text) - Warranty details
  - `marca` (text) - Brand name
  - `numero_lacre` (text) - Seal number
  - `numero_placa` (text) - Plate number
  - `adesivo` (text) - Sticker identifier
  - `usuario` (text) - Assigned user name
  - `obra` (text) - Assigned project name
  - `created_at` (timestamptz) - Creation timestamp
  - `updated_at` (timestamptz) - Last update timestamp
  ...
  ### 6. `obra_images`
  Stores multiple images per construction project
  - `id` (uuid, primary key) - Unique image identifier
  - `obra_id` (uuid) - Reference to construction project
  - `image_url` (text) - Image URL
  - `description` (text) - Image description
  - `display_order` (integer) - Display order for gallery
...
-- Obra images policies
CREATE POLICY "Users can view images from their host's obras"
  ON obra_images FOR SELECT
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM obras o
      WHERE o.id = obra_id AND
      (o.owner_id = auth.uid() OR o.owner_id IN (SELECT host_id FROM users WHERE id = auth.uid()))
    )
  );

CREATE POLICY "Users can insert images to their host's obras"
  ON obra_images FOR INSERT
  TO authenticated
  WITH CHECK (
...
CREATE POLICY "Users can delete images from their host's obras"
  ON obra_images FOR DELETE
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM obras o
