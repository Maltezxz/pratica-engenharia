...
export default function DesaparecidosPage() {
  ...
  const markAsFound = (ferramenta: Ferramenta) => {
    showConfirm({
      title: 'Marcar como Encontrado',
      message: `Tem certeza que deseja marcar "${ferramenta.name}" como encontrado?`,
      confirmText: 'Marcar',
      type: 'info',
      onConfirm: () => executeMarkAsFound(ferramenta),
    });
  };

  const executeMarkAsFound = async (ferramenta: Ferramenta) => {
    try {
      ...
      await loadData();
      showToast('success', 'Equipamento marcado como encontrado com sucesso!');
    } catch (error) {
